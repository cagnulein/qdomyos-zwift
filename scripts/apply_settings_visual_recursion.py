...
replacements = [
    (
        ...
        "modernCategoryList visibility",
    ),
    (
        '                    visible: modernSettingsParent.length > 0 || modernSettingsSearch.text.length > 0\n',
        '                    visible: modernSettingsParent.length > 0 || modernSettingsSearch.text.length > 0 || modernSettingsExternalTarget.length > 0\n',
        "modernItemList visibility",
    ),
    (
...
for old, new, label in replacements:
    ...
    if count != 1:
        raise SystemExit(f"unexpected {label} match count: {count}")
    text = text.replace(old, new, 1)

start_marker = "        // SETTINGS_QML_VISUAL_HARNESS_V1\n"
end_marker = "        Component.onCompleted: {\n"
if text.count(start_marker) != 1:
    raise SystemExit(f"unexpected V1 harness marker count: {text.count(start_marker)}")
start = text.index(start_marker)
end = text.index(end_marker, start)

harness = r'''        // SETTINGS_QML_VISUAL_HARNESS_V2
...
text = text[:start] + harness + text[end:]
