...
void ypooelliptical::forceResistance(resistance_t requestResistance) {
    ...
    if(E35 || SCH_590E || KETTLER || CARDIOPOWER_EEGO || MYELLIPTICAL || SKANDIKA || DOMYOS || FEIER) {
...
void ypooelliptical::characteristicChanged(const QLowEnergyCharacteristic &characteristic, const QByteArray &newvalue) {
    ...
    if (characteristic.uuid() == QBluetoothUuid((quint16)0x2ACE) && !iconsole_elliptical) {
        ...
        if(E35 == false && SCH_590E == false && KETTLER == false && CARDIOPOWER_EEGO == false && MYELLIPTICAL == false && SKANDIKA == false && DOMYOS == false && FEIER == false) {
        ...
        if (Flags.avgPower && lastPacket.length() > index + 1 && !E35 && !SCH_590E && !KETTLER && !CARDIOPOWER_EEGO && !MYELLIPTICAL && !SKANDIKA && !DOMYOS && !FEIER) { // E35 has a bug about this
            double avgPower;
...
void ypooelliptical::stateChanged(QLowEnergyService::ServiceState state) {
    ...
    for (QLowEnergyService *s : qAsConst(gattCommunicationChannelService)) {
        ...
        else if(s->serviceUuid() != _gattFTMSService && (SCH_590E || MYELLIPTICAL || SKANDIKA || DOMYOS || FEIER)) {
            qDebug() << "skipping service" << s->serviceUuid();
            continue;            
        }
