...
fakerower::fakerower(bool noWriteResistance, bool noHeartService, bool noVirtualDevice) {
    ...
    initDone = false;
    connect(refresh, &QTimer::timeout, this, &fakerower::update);
    refresh->start(200ms);
}
    ...
    if (!firstStateChanged && !this->hasVirtualDevice() && !noVirtualDevice
        ...
        if (ios_peloton_workaround && cadence && !virtual_device_rower) {
            ...
            if (!virtual_device_rower) {
                ...
                auto virtualBike = new virtualbike(this, noWriteResistance, noHeartService, 0, 1);
...
void fakerower::ftmsCharacteristicChanged(const QLowEnergyCharacteristic &characteristic, const QByteArray &newValue) {
    QByteArray b = newValue;
    qDebug() << "routing FTMS packet to the bike from virtualbike" << characteristic.uuid() << newValue.toHex(' ');
}

void fakerower::changeInclinationRequested(double grade, double percentage) {
    if (percentage < 0)
        percentage = 0;
