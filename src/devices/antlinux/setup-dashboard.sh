...
show_scrollable_menu_fast() {
    ...
    while true; do
        ...
        local help_w
        help_w=$(get_vis_width "$help_text")
        local left_pad="═══  "
        local right_pad="══"
        local left_w=5
        local right_w=2
        ...
        local sep_w=2
        local fill_w=$((INNER_COLS - left_w - help_w - sep_w - right_w))
        [[ $fill_w -lt 0 ]] && fill_w=0

        local footer_fill
        footer_fill=$(printf '═%.0s' $(seq 1 "$fill_w"))

        # Include a plain two-space separator between help_text and fill
        # Use builder to produce the footer row string and append to the
        # atomic render buffer so all footers come from the same codepath.
        render_buffer+=$(build_hr_string "$b_row" "╚" "╝" "$help_text" "${BOLD_BLUE}" "")
