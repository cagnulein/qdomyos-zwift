...
void bluetoothdevice::processSlopeChange(int16_t iresistance, uint8_t crr, uint8_t cw,
                                          double &outGrade, double &outPercentage,
                                          double bikeResistanceGain, int8_t bikeResistanceOffset) {
    BLUETOOTH_TYPE dt = deviceType();
    QSettings settings;
    ...
    bool zwift_play_emulator =
        settings.value(QZSettings::zwift_play_emulator, QZSettings::default_zwift_play_emulator).toBool();
    double min_inclination =
        settings.value(QZSettings::min_inclination, QZSettings::default_min_inclination).toDouble();
    ...
    */
    ...
    if (dt == BIKE) {
        ...
        // from Zwift
        if (!((bike *)this)->inclinationAvailableByHardware()) {
            if(zwift_play_emulator)
                setInclination(grade);
            else
                setInclination(grade + CRR_offset + CW_offset);
        }
        if (force_resistance && !erg_mode) {
            // same on the training program
            changeResistance((resistance_t)(round(resistance * bikeResistanceGain)) + bikeResistanceOffset + 1 +
                             CRR_offset + CW_offset); // resistance start from 1
        }
    } else if (dt == ELLIPTICAL) {
        bool inclinationAvailableByHardware = ((elliptical *)this)->inclinationAvailableByHardware();
