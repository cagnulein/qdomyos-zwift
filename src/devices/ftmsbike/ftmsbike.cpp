...
void ftmsbike::deviceDiscovered(const QBluetoothDeviceInfo &device) {
               ...
               device.address().toString() + ')');
    {
        ...
        if (bluetoothDevice.name().toUpper().startsWith("SUITO")) {
            ...
            ergModeSupported = false;
            max_resistance = 32;
            // Pre-populate ergTable with calibrated power curve (9 cadences x 32 levels = 288 points)
            // Measured at 80 RPM, extrapolated to 60-100 RPM using P ∝ cadence^1.1
            _ergTable.loadDefaultData(QStringLiteral(
