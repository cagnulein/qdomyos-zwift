...
// Calories burned per minute on a treadmill, using the ACSM metabolic equations.
// Unlike the Keytel cycling formula, this is linear in body weight (the power on a
// treadmill is generated by moving the body, so it must not be multiplied by weight twice).
double treadmill::caloriesPerMinuteCalc(double weight, double speed, double inclination) {
    if (speed <= 0)
        return 0;
    double speed_m_min = speed * 1000.0 / 60.0; // from km/h to m/min
    double grade = inclination / 100.0;
    double VO2; // mL/kg/min
    if (speed < 8.0)
        VO2 = 0.1 * speed_m_min + 1.8 * speed_m_min * grade + 3.5; // ACSM walking
    else
        VO2 = 0.2 * speed_m_min + 0.9 * speed_m_min * grade + 3.5; // ACSM running
    if (VO2 < 3.5) // never go below resting metabolic rate (e.g. steep downhill)
        VO2 = 3.5;
    return (VO2 * weight * 5.0) / 1000.0; // 1 L O2 ~= 5 kcal; VO2 in mL/kg/min * kg / 1000 -> L/min
}

// Returns the KCal increment for the given elapsed time, picking the right model:
// - with a real power sensor the measured watts are weight independent, so the Keytel
//   formula (calibrated for cycling ergometers) is appropriate;
// - otherwise the watts are computed from body weight, so we use the ACSM equations
//   to avoid counting the weight twice (which over-estimated calories for heavy users).
double treadmill::calculateKCalChange(double weight, double elapsedMs) {
    if (elapsedMs <= 0 || currentSpeed().value() <= 0)
        return 0;
    ...
    if (powerReceivedFromPowerSensor) {
        ...
        if (w == 0)
            return 0;
        kcalPerMinute = ((0.048 * w + 1.19) * weight * 3.5) / 200.0;
    } else {
        kcalPerMinute = caloriesPerMinuteCalc(weight, currentSpeed().value(), currentInclination().value());
    }
    return kcalPerMinute / (60000.0 / elapsedMs);
}
