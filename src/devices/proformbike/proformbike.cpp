...
void proformbike::forceResistance(resistance_t requestResistance) {
    if(proform_bike_PFEVEX71316_0) {
        ...
        // NordicTrack VR21 needs the noop command before resistance change
        if (nordictrack_vr21) {
            const uint8_t noop[] = {0xfe, 0x02, 0x0d, 0x02};
            writeCharacteristic((uint8_t *)noop, sizeof(noop), QStringLiteral("resrequest"), false, false);
        }
