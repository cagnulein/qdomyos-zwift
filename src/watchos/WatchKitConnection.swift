...
extension WatchKitConnection: WCSessionDelegate {
    func session(_ session: WCSession, activationDidCompleteWith activationState: WCSessionActivationState, error: Error?) {
        print("activationDidCompleteWith - watchOS")

        // Check if there's a received application context with tile settings
        if !session.receivedApplicationContext.isEmpty {
            updateTileSettings(from: session.receivedApplicationContext)
        }
    }

    /// Receive tile settings from iOS app via application context
    func session(_ session: WCSession, didReceiveApplicationContext applicationContext: [String : Any]) {
        print("Received application context with \(applicationContext.count) settings")
        updateTileSettings(from: applicationContext)
    }

    /// Update tile configuration from received settings
    private func updateTileSettings(from settings: [String : Any]) {
        DispatchQueue.main.async {
            TileConfiguration.shared.updateFromSettings(settings)
            print("Tile configuration updated on watch")
        }
    }
