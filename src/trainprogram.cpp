...
// Test-friendly scheduler that allows forcing the tick value
// This enables unit tests to simulate specific ticks without relying on real-time timers
void trainprogram::scheduler(int tick) {
    ...
    // Force the tick value for testing
    ticks = tick;

    // Get odometer from device (will be 0.0 for test mocks)
    double odometerFromTheDevice = 0.0;
    if (bluetoothManager && bluetoothManager->device()) {
        odometerFromTheDevice = bluetoothManager->device()->odometer();
    }

    // Main scheduler logic (copy from scheduler())
    uint32_t calculatedLine;
    uint32_t calculatedElapsedTime = 0;
    for (calculatedLine = 0; calculatedLine < static_cast<uint32_t>(rows.length()); calculatedLine++) {
        calculatedElapsedTime += calculateTimeForRow(calculatedLine);
    ...
    // Check if we've reached the end of all rows
    if (calculatedLine >= static_cast<uint32_t>(rows.length())) {
        // Calculate total workout time
        uint32_t totalTime = 0;
        for (int i = 0; i < rows.length(); i++) {
        ...
        // Only call end() if current tick EXCEEDS total time
        if (static_cast<uint32_t>(ticks) > totalTime) {
            end();
            return;
        }
