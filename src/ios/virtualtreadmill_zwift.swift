...
class BLEPeripheralManagerZwift: NSObject, CBPeripheralManagerDelegate {
  ...
  private var pendingServicesToAdd: Int = 0  // Fix: track pending add() calls before advertising
    ...
    case .poweredOn:
      print("Peripheral manager is up and running")
        ...
        if(!self.garmin_bluetooth_compatibility) {
            ...
            // Fix: count all services we are about to add so we only start
            // advertising once ALL are registered in the GATT database.
            // Previously, startAdvertising fired after the first add() (heartRateService)
            // before FTMS/CSC/Power were added, causing Android to miss those services.
            self.pendingServicesToAdd = 4  // heartRate, FitnessMachine, CSC, Power
            if self.zwift_play_emulator || self.watt_bike_emulator {
                self.pendingServicesToAdd += 1
            }
  ...
  func peripheralManager(_ peripheral: CBPeripheralManager, didAdd service: CBService, error: Error?) {
    ...
    // Fix: only start advertising once ALL services are registered in GATT.
    // Previously this fired after every add(), so advertising started before
    // FTMS/CSC/Power were in the GATT database and Android missed them on discovery.
    if pendingServicesToAdd > 0 {
        pendingServicesToAdd -= 1
    }
    guard pendingServicesToAdd == 0 else {
        print("Waiting for \(pendingServicesToAdd) more service(s) before advertising")
        return
    }
