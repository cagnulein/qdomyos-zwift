...
void toorxtreadmill::deviceDiscovered(const QBluetoothDeviceInfo &device) {
    ...
    if (device.name().startsWith("TRX ROUTE KEY")) {
        ...
        connect(discoveryAgent, SIGNAL(serviceDiscovered(QBluetoothServiceInfo)), this,
...
void toorxtreadmill::serviceDiscovered(const QBluetoothServiceInfo &service) {
    ...
    if (service.device().address() == bluetoothDevice.address()) {
        debug("Found new service: " + service.serviceName() + '(' + service.serviceUuid().toString() + ')');
...
uint8_t toorxtreadmill::GetHeartRateFromPacket(QByteArray packet) { return packet.at(16); }

uint8_t toorxtreadmill::GetInclinationFromPacket(QByteArray packet) { return packet.at(15); }
