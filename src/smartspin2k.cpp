...
void smartspin2k::calibrateShiftStep() {
    ...
    if (nSamples == 0) {
        slope = 0;
        intercept = settings.value(QZSettings::ss2k_shift_step, QZSettings::default_ss2k_shift_step /* 900 */).toUInt();
        return;
    } else if (nSamples == 1) {
        slope = 0;
        intercept = y[0];
...
void smartspin2k::forceResistance(resistance_t requestResistance) {
    ...
    double ss2k_min_resistance = settings.value(QZSettings::ss2k_min_resistance, QZSettings::default_ss2k_min_resistance /* 0 */).toDouble();
    double ss2k_max_resistance = settings.value(QZSettings::ss2k_max_resistance, QZSettings::default_ss2k_max_resistance /* 100 */).toDouble();
