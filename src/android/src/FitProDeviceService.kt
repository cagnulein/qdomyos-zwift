...
object FitProDeviceService {
    ...
    @Volatile private var workoutStateInitialized = false
    ...
    private fun updateWorkoutState(rawMode: Int?) {
        ...
        if (!workoutStateInitialized) {
            workoutStateInitialized = true
            currentWorkoutState = mapped
            stateChangeQueue.add(intArrayOf(0, mapped))
            QLog.i(TAG, "Initial workout state received: $mapped (raw=$rawMode)")
            return
        }
