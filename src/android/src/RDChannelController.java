...
public class RDChannelController {
    // The device type and transmission type to be part of the channel ID message
    private static final int CHANNEL_SPEED_DEVICE_TYPE = 0x1e;
    private static final int CHANNEL_SPEED_TRANSMISSION_TYPE = 1;

    // The period and frequency values the channel will be configured to
    private static final int CHANNEL_SPEED_PERIOD = 4096; // 4 Hz
    private static final int CHANNEL_SPEED_FREQUENCY = 57;

    private static final String TAG = RDChannelController.class.getSimpleName();
    public static final int SPEED_SENSOR_ID = 0x9e3d4b0a;
    ...
    public RDChannelController(AntChannel antChannel) {
               ...
               carousalTimer.scheduleAtFixedRate(new TimerTask() {
                   ...
                   public void run() {
                       ...
                       if(page == 0) {
                           page = 1;
                           payload[0] = (byte) 0x00;
                           payload[1] = (byte) cadence;
                           payload[2] = (byte) 0;
                           payload[3] = (byte) 0;
                           payload[4] = (byte) 0;
                           payload[5] = (byte) 0;
                           payload[6] = (byte) 0;
                           payload[7] = (byte) (((stride_count++) << 1) & 0xFF); // bad
                       } else {
                           page = 0;
                           payload[0] = (byte) 0x010;
                           byte v = (byte) (speed / 3.6);
                           double fract = (speed / 3.6) - ((double)v) * 256.0;
                           payload[1] = (byte)((byte) ((byte)(v & 0x0F)) | (byte)(((((byte)fract) & 0x0F) << 4) & 0xF0));
                           payload[2] = (byte) ((((((byte)fract) & 0xF0) >> 4) & 0x0F) | 0xF0);
                           payload[3] = (byte) 0xFF;
                           payload[4] = (byte) 0xFF;
                           payload[5] = (byte) 0xFF;
                           payload[6] = (byte) 0xFF;
