...
void trxappgateusbbike::writeCharacteristic(uint8_t *data, uint8_t data_len, QString info, bool disable_log,
...
void trxappgateusbbike::serviceDiscovered(const QBluetoothUuid &gatt) { debug("serviceDiscovered " + gatt.toString()); }
...
*/
    ...
    if (bike_type != JLL_IC400) {
        ...
        if (Heart.value() > 0) {
            int avgP =
                ((settings.value("power_hr_pwr1", 200).toDouble() * settings.value("power_hr_hr2", 170).toDouble()) -
                 (settings.value("power_hr_pwr2", 230).toDouble() * settings.value("power_hr_hr1", 150).toDouble())) /
                    (settings.value("power_hr_hr2", 170).toDouble() - settings.value("power_hr_hr1", 150).toDouble()) +
                (Heart.value() *
                 ((settings.value("power_hr_pwr1", 200).toDouble() - settings.value("power_hr_pwr2", 230).toDouble()) /
                  (settings.value("power_hr_hr1", 150).toDouble() - settings.value("power_hr_hr2", 170).toDouble())));
            if (Speed.value() > 0)
                watt = avgP;
            else
                watt = 0;
...
void trxappgateusbbike::btinit(bool startTape) {
    ...
    if (toorx30 == false && (bike_type == TYPE::IRUNNING || bike_type == TYPE::ICONSOLE)) {
        ...
        writeCharacteristic((uint8_t *)initData1, sizeof(initData1), "init", false, false);
        writeCharacteristic((uint8_t *)initData1, sizeof(initData1), "init", false, true);
        writeCharacteristic((uint8_t *)initData2, sizeof(initData2), "init", false, true);
        writeCharacteristic((uint8_t *)initData3, sizeof(initData3), "init", false, false);
        writeCharacteristic((uint8_t *)initData4, sizeof(initData4), "init", false, false);
        writeCharacteristic((uint8_t *)initData4, sizeof(initData4), "init", false, false);
        writeCharacteristic((uint8_t *)initData5, sizeof(initData5), "init", false, true);
...
void trxappgateusbbike::errorService(QLowEnergyService::ServiceError err) {
    ...
    debug("trxappgateusbbike::errorService" + QString::fromLocal8Bit(metaEnum.valueToKey(err)) +
