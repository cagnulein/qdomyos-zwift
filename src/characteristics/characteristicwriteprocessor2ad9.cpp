...
int CharacteristicWriteProcessor2AD9::writeProcess(quint16 uuid, const QByteArray &data, QByteArray &reply) {
    if (data.size()) {
        ...
        if (dt == BIKE || dt == ROWING) {
            ...
            QSettings settings;
            // When true, speed/incline commands from the connected app are forwarded to the device.
            bool force_speed =
            ...
            } else if ((char)data.at(0) == 0x07) // Start request
            {
                ...
                if (dt == TREADMILL) {
#ifdef ANT_LINUX_ENABLED
                    // Guard on speed==0 to avoid re-initialising the belt when already running.
                    if (Bike->currentSpeed().value() == 0)
#endif
                        Bike->start();
                }
                qDebug() << QStringLiteral("request to start/resume");
            } else if ((char)data.at(0) == 0x08) // Stop or Pause
            {
                // Bike->stop();
                if (dt == TREADMILL)
                    Bike->stop(false);
