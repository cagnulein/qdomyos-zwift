...
void peloton::startEngine() {
    ...
    obj[QStringLiteral("username_or_email")] =
        settings.value(QZSettings::peloton_username, QZSettings::default_peloton_username /* QStringLiteral("username") */).toString();
    obj[QStringLiteral("password")] =
        settings.value(QZSettings::peloton_password, QZSettings::default_peloton_password /* QStringLiteral("password") */).toString();
...
void peloton::instructor_onfinish(QNetworkReply *reply) {
    ...
    if (settings.value(QZSettings::peloton_date, QZSettings::default_peloton_date /* QStringLiteral("Before Title") */)
            .toString()
            .contains(QStringLiteral("Before"))) {
        workout_name = air_time + QStringLiteral(" ") + workout_name;
    } else if (settings.value(QZSettings::peloton_date, QZSettings::default_peloton_date /* QStringLiteral("Before Title") */)
                   .toString()
...
void peloton::ride_onfinish(QNetworkReply *reply) {
    ...
    QSettings settings;
    QString difficulty = settings.value(QZSettings::peloton_difficulty, QZSettings::default_peloton_difficulty /* QStringLiteral("lower") */).toString();
            ...
            if (!subsegments_v2.isEmpty()) {
                foreach (QJsonValue s, subsegments_v2) {
                    ...
                    if (!zone.toUpper().compare(QStringLiteral("SPIN UPS"))) {
                        ...
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 0.50;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 0.75;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 0.66;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 0.83;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 0.98;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 1.13;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 1.35;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
                        ...
                        r.duration = QTime(0, len / 60, len % 60, 0);
                        r.power = settings.value(QZSettings::ftp, QZSettings::default_ftp /* 200.0 */).toDouble() * 1.5;
                        if (r.power != -1) {
                            atLeastOnePower = true;
                        }
                        trainrows.append(r);
