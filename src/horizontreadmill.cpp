...
void horizontreadmill::serviceDiscovered(const QBluetoothUuid &gatt) { debug("serviceDiscovered " + gatt.toString()); }
...
void horizontreadmill::characteristicChanged(const QLowEnergyCharacteristic &characteristic,
    ...
    if (Flags.expEnergy) {
        ...
        KCal += ((((0.048 * ((double)watts(settings.value("weight", 75.0).toFloat())) + 1.19) * 3.5) / 200.0) /
...
void horizontreadmill::stateChanged2(QLowEnergyService::ServiceState state) {
    ...
    connect(gattCommunication2ChannelService, SIGNAL(characteristicChanged(QLowEnergyCharacteristic, QByteArray)), this,
...
void horizontreadmill::stateChanged(QLowEnergyService::ServiceState state) {
    ...
    connect(gattCommunicationChannelService, SIGNAL(characteristicChanged(QLowEnergyCharacteristic, QByteArray)), this,
            SLOT(characteristicChanged(QLowEnergyCharacteristic, QByteArray)));
...
void horizontreadmill::serviceScanDone(void) {
    ...
    connect(gattCommunication2ChannelService, SIGNAL(stateChanged(QLowEnergyService::ServiceState)), this,
            SLOT(stateChanged2(QLowEnergyService::ServiceState)));
...
void horizontreadmill::errorService(QLowEnergyService::ServiceError err) {
    ...
    debug("horizontreadmill::errorService" + QString::fromLocal8Bit(metaEnum.valueToKey(err)) +
