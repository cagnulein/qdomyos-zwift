...
void schwinnic4bike::characteristicChanged(const QLowEnergyCharacteristic &characteristic, const QByteArray &newValue) {
    ...
    if (settings.value(QZSettings::schwinn_bike_resistance, QZSettings::default_schwinn_bike_resistance /* false */).toBool())
...
void schwinnic4bike::stateChanged(QLowEnergyService::ServiceState state) {
    ...
    if (!firstStateChanged && !virtualBike
        ...
        if (ios_peloton_workaround && cadence) {
            ...
            uint8_t bikeResistanceOffset = settings.value(QZSettings::bike_resistance_offset, QZSettings::default_bike_resistance_offset /* 4 */).toInt();
            double bikeResistanceGain = settings.value(QZSettings::bike_resistance_gain_f, QZSettings::default_bike_resistance_gain_f /* 1.0 */).toDouble();
            virtualBike =
...
resistance_t schwinnic4bike::pelotonToBikeResistance(int pelotonResistance) {
    QSettings settings;
    bool schwinn_bike_resistance_v2 = settings.value(QZSettings::schwinn_bike_resistance_v2, QZSettings::default_schwinn_bike_resistance_v2 /* false */).toBool();
