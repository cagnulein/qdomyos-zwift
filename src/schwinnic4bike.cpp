...
void schwinnic4bike::serviceDiscovered(const QBluetoothUuid &gatt) { debug("serviceDiscovered " + gatt.toString()); }
...
void schwinnic4bike::characteristicChanged(const QLowEnergyCharacteristic &characteristic, const QByteArray &newValue) {
    ...
    m_pelotonResistance =
        (((sqrt(pow(br, 2.0) -
           ...
           br) /
          (2.0 * ar)) *
         settings.value("peloton_gain", 1.0).toDouble()) +
        settings.value("peloton_offset", 0.0).toDouble();
