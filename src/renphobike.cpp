...
void renphobike::writeCharacteristic(uint8_t* data, uint8_t data_len, QString info, bool disable_log, bool wait_for_response)
{
    QEventLoop loop;
    QTimer timeout;
...
void renphobike::update()
{
    ...
    if(initRequest)
    {
        ...
        writeCharacteristic(write, sizeof(write), "requestControl " + QString::number(requestResistance));
        initRequest = false;
    }
...
void renphobike::serviceDiscovered(const QBluetoothUuid &gatt)
{
...
void* renphobike::VirtualBike()
{
    return virtualBike;
}

void* renphobike::VirtualDevice()
{
    return VirtualBike();
}
