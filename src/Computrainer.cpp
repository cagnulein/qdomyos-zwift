...
const static uint8_t ergo_command[56] =
    {
        ...
        //      ----  ----  ----  ----  ----  ----  ----
        0x6D, 0x00, 0x00, 0x0A, 0x08, 0x00, 0xE0, 0x65, 0x00, 0x00, 0x0A, 0x10, 0x00, 0xE0,
        0x00, 0x00, 0x00, 0x0A, 0x18, 0x5D, 0xC1, 0x33, 0x00, 0x00, 0x0A, 0x24, 0x1E, 0xE0,
        0x6A, 0x00, 0x00, 0x0A, 0x2C, 0x5F, 0xE0, 0x41, 0x00, 0x00, 0x0A, 0x34, 0x00, 0xE0,
        0x2D, 0x00, 0x00, 0x0A, 0x38, 0x10, 0xC2, 0x03, 0x00, 0x00, 0x0A, 0x40, 0x32, 0xE0 // set LOAD command
};
...
const static uint8_t ss_command[56] = {
    ...
    0x74, 0x00, 0x00, 0x16, 0x18, 0x5D, 0xC1, 0x27, 0x00, 0x00, 0x16, 0x24, 0x1E, 0xE0,
    0x5E, 0x00, 0x00, 0x16, 0x2C, 0x5F, 0xE0, 0x35, 0x00, 0x00, 0x16, 0x34, 0x00, 0xE0,
    0x21, 0x00, 0x00, 0x16, 0x38, 0x10, 0xC2, 0x29, 0x00, 0x00, 0x16, 0x40, 0x00, 0xE0};

const static uint8_t rrc_command[56] = {

    0x31, 0x00, 0x00, 0x0e, 0x40, 0x00, 0xe0, 0x69, 0x00, 0x00, 0x0e, 0x08, 0x00, 0xe0, 0x61, 0x00, 0x00, 0x0e, 0x10,
    0x00, 0xe0, 0x78, 0x00, 0x00, 0x0e, 0x18, 0x61, 0xc1, 0x4d, 0x00, 0x00, 0x0e, 0x24, 0x00, 0xe0, 0x6e, 0x00, 0x00,
    0x0e, 0x2c, 0x57, 0xc1, 0x3d, 0x00, 0x00, 0x0e, 0x34, 0x00, 0xe0, 0x23, 0x00, 0x00, 0x0e, 0x38, 0x16, 0xc2
...
void Computrainer::getTelemetry(double &power, double &heartrate, double &cadence, double &speed, double &RRC,
                                bool &calibration, int &buttons, uint8_t *ss, int &status) {
...
void Computrainer::prepareCommand(int mode, double value) {
    ...
    case CT_SSMODE:
        ...
        if (value < -9.9)
            ...
            gradient = value * 10; // gradient is passed as an integer
...
int Computrainer::calcCRC(int value) { return (0xff & (107 - (value & 0xff) - (value >> 8))); }
        ...
        if (isDeviceOpen == true) {
            ...
            if (readMessage() > 0) {
                ...
                if (buttons != curButtons) {
                    ...
                    pvars.lock();
                    this->deviceButtons |= buttons; // Borrowed from Fortius.cpp: workaround to ensure controller
                                                    // doesn't miss button pushes
                    pvars.unlock();
                }
...
void CTsleeper::msleep(unsigned long msecs) { QThread::msleep(msecs); }
...
bool Computrainer::discover(QString filename) {
    ...
    handshake[6] = '\0';
    if (strcmp((char *)handshake, "LinkUp"))
        return false;
    else
        return true;
}
