...
void spirittreadmill::writeCharacteristic(uint8_t *data, uint8_t data_len, QString info, bool disable_log,
...
void spirittreadmill::serviceDiscovered(const QBluetoothUuid &gatt) { debug("serviceDiscovered " + gatt.toString()); }
...
void spirittreadmill::errorService(QLowEnergyService::ServiceError err) {
    ...
    debug("spirittreadmill::errorService" + QString::fromLocal8Bit(metaEnum.valueToKey(err)) +
