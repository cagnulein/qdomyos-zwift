...
void eslinkertreadmill::writeCharacteristic(uint8_t *data, uint8_t data_len, QString info, bool disable_log,
...
void eslinkertreadmill::serviceDiscovered(const QBluetoothUuid &gatt) { debug("serviceDiscovered " + gatt.toString()); }
...
void eslinkertreadmill::errorService(QLowEnergyService::ServiceError err) {
    ...
    debug("eslinkertreadmill::errorService " + QString::fromLocal8Bit(metaEnum.valueToKey(err)) +
