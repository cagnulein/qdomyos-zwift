...
function show_osd(cmd,client,arg)
    if arg then
        narg = 0
        st = ''
        pause = 1000000
        ...
        if vlc.osd==nil then
            client:append('OSD NIL')
        else
            vlc.osd.message(st,nil,position,pause)
        end
    end
end
...
commands_ordered = {
    ...
    { "osd"; { func = show_osd; args = "[X]"; help = "Show OSD" } };
    }
